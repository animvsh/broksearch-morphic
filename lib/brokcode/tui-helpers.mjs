...
const VALID_USAGE_PERIODS = new Set(['day', 'week', 'month'])
...
export function isValidBrokKey(key) {
  ...
  if (!key.startsWith(BROK_KEY_PREFIX)) return false
  const suffix = key.slice(BROK_KEY_PREFIX.length)
...
export function normalizeBrokKey(key) {
  ...
  if (!isValidBrokKey(trimmed)) return null
  return trimmed
}

export function normalizeUsagePeriod(period) {
  if (typeof period !== 'string') return 'day'
  const normalized = period.trim().toLowerCase()
  return VALID_USAGE_PERIODS.has(normalized) ? normalized : 'day'
}
 ...
 * @returns {boolean}
 */
export function isDangerousShellCommand(cmd) {
  if (typeof cmd !== 'string') return false
  return DANGEROUS_SHELL_PATTERNS.some(pattern => pattern.test(cmd))
}

/**
 * Extracts the first fenced code block from a Brok response.
 * Strips the trailing newline that follows the opening fence.
 * @param {string} text
 * @returns {string | null}
 */
export function extractFencedCodeBlock(text) {
  ...
  // Strip the single trailing newline that follows the opening fence
